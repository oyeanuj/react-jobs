...
#### Arguments
  ...
  - `config` (_Object_) : The configuration object for the async Component. It has the following properties available:
    ...
    - `onlyUseWrappedcomponent` (_Boolean_, default: `false`): If set to true, it will pass the job state to the wrapped component and **ignore** the LoadingComponent and ErrorComponent. Pass this prop, if you want loading and error states handled by the wrapped component rather than separate components.
